...
const PreferencesDialog: React.FC<PreferencesDialogProps> = ({ onClose }) => {
  ...
  const [streamingEnabled, setStreamingEnabled] = useState(() => {
    return localStorage.getItem('RiddimSync_streaming_enabled') !== 'false';
  });
  const [bufferSize, setBufferSize] = useState(() =>
    localStorage.getItem('RiddimSync_buffer_size') ?? '256'
  );
  ...
  const handleSave = () => {
    localStorage.setItem('RiddimSync_streaming_enabled', String(streamingEnabled));
    localStorage.setItem('RiddimSync_buffer_size', bufferSize);
    localStorage.setItem('RiddimSync_output_device', selectedOutput);
    localStorage.setItem('RiddimSync_input_device', selectedInput);
    onClose();
  };
